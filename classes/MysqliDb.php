...
class MysqliDb
{
    ...
    */
    ...
    ########################### brahham ###########################
    public function almacenar($sTabla, $aDatos, $aValidadores=null){
        // Se debe verificar que los valores del validador no tenga datos repetidos
        // en la tabla seleccionada
        if (is_array($aValidadores) && count($aValidadores)>0) {
            foreach ($aValidadores as $campo => $validador) {
                $this->where($campo, $validador);
            }
            $aResp = $this->get($sTabla);
        } else {
            $aResp = null;
        }
        // Si existen datos mandar retornar false
        if (is_array($aResp) && count($aResp)>0) {
            return false;
        } else {
            // Si no existen datos repetidos almacenar la informacion correspondiente
            $idResp = $this->insert($sTabla, $aDatos);
            return $idResp;
        }
    }
